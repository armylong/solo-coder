...
export class Game {
    ...
    start() {
        ...
        this.players = [
            new Player(0, '你', false, 'A'),
            new Player(1, '电脑1', true, 'B'),
            new Player(2, '电脑2', true, 'A'),
            new Player(3, '电脑3', true, 'B'),
            new Player(4, '电脑4', true, 'A'),
            new Player(5, '电脑5', true, 'B')
        ];

        let cardIndex = 0;
        while (this.deck.cards.length > 0) {
            this.players[cardIndex % 6].addCard(this.deck.deal());
            cardIndex++;
        }
    ...
    _playCards(playerIndex, cards, type) {
        ...
        if (player.cards.length === 0) {
            this.finishOrder.push({ player: playerIndex, team: player.team });
            this._checkGameOver();
            return;
        }
    ...
    _checkGameOver() {
        const teamAFinished = this.finishOrder.filter(f => f.team === 'A').length;
        const teamBFinished = this.finishOrder.filter(f => f.team === 'B').length;

        if (teamAFinished >= 2 || teamBFinished >= 2) {
            this.state = GAME_STATES.GAME_OVER;
            if (teamAFinished >= 2) {
                this.teamScores.A = 3;
                this.teamScores.B = this.finishOrder.filter(f => f.team === 'B').length;
            } else {
                this.teamScores.B = 3;
                this.teamScores.A = this.finishOrder.filter(f => f.team === 'A').length;
            }
            this._render();
            return;
        }
    ...
    _nextPlayer() {
        let nextIndex = (this.currentIndex + 1) % 6;
        let attempts = 0;

        while (this.players[nextIndex].cards.length === 0 && attempts < 6) {
    ...
    _render() {
        ...
        if (this.state === GAME_STATES.WAITING) {
            this.renderer.drawTitle('够级 - 山东扑克');
            this.renderer.drawButton(this.canvas.width / 2 - 60, this.canvas.height / 2 - 20, 120, 40, '开始游戏');
            return;
        }

        this.renderer.drawPlayers(this.players, this.currentIndex, this.finishOrder);
        ...
        if (this.state === GAME_STATES.GAME_OVER) {
            const winner = this.teamScores.A > this.teamScores.B ? 'A队' : 'B队';
            this.renderer.drawGameOver(`${winner}获胜！`);
