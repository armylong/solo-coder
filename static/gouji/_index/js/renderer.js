...
export class Renderer {
    ...
    drawPlayers(players, currentIndex, finishOrder) {
        const positions = [
            { x: this.canvas.width / 2, y: this.canvas.height - 100, labelY: this.canvas.height - CARD_HEIGHT - 130 },
            { x: 100, y: this.canvas.height / 2, labelY: this.canvas.height / 2 - 50 },
            { x: 250, y: 100, labelY: 50 },
            { x: this.canvas.width / 2, y: 60, labelY: 20 },
            { x: this.canvas.width - 250, y: 100, labelY: 50 },
            { x: this.canvas.width - 100, y: this.canvas.height / 2, labelY: this.canvas.height / 2 - 50 }
        ];

        for (let i = 0; i < players.length; i++) {
            const player = players[i];
            const pos = positions[i];
            const isFinished = finishOrder.some(f => f.player === i);

            this.ctx.fillStyle = player.team === 'A' ? COLORS.TEAM_A : COLORS.TEAM_B;
            this.ctx.font = 'bold 14px Arial';
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'middle';
