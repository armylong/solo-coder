...
func ChangePassword(uid int64, req *ChangePasswordRequest) error {
	if req.OldPassword == "" {
		return errcode.InvalidParam("旧密码不能为空")
	}
	if req.NewPassword == "" {
		return errcode.InvalidParam("新密码不能为空")
	}
	if req.ConfirmPassword == "" {
		return errcode.InvalidParam("确认密码不能为空")
	}
	if req.NewPassword != req.ConfirmPassword {
		return errcode.InvalidParam("两次输入的新密码不一致")
	}
	if len(req.NewPassword) < 6 {
		return errcode.InvalidParam("新密码长度至少为6位")
	}
	...
	if err != nil {
		return errcode.InvalidParam("旧密码错误")
	}
